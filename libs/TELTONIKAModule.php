...
eval('declare(strict_types=1);namespace TELTONIKA {?>' . file_get_contents(__DIR__ . '/SymconModulHelper/VariableProfileHelper.php') . '}');

/**
 * TELTONIKA ist die Basisklasse für alle Router der Firma TELTONIKA Networks
 * Erweitert ipsmodule.
 * @property array $Variables
 */
class TELTONIKA extends IPSModule
{
    use \TELTONIKA\SemaphoreHelper;
    use \TELTONIKA\VariableProfileHelper;
    const Swap = true;
    ...
    {
        parent::ApplyChanges();
        $this->RegisterProfileFloat('GPSCords', 'Move', '', ' ', 0, 0, 0, 6);
        $this->RegisterProfileInteger('RSSI', '', '', ' dBm', 0, 0, 0);
    ...
    {
        switch ($Variable['ConvertionType']) {
            case 'BOOLEAN':
            ...
            case 'FLOAT_Temp':
                switch ($Variable['Quantity']) {
                    case 2:
                        $TempRaw = unpack('l', $Value)[1];
                        $TempInt = intval($TempRaw/10);
                        $TempReturn = floatval($TempInt);
                        return $TempReturn;
                }
                break;
            case 'STRING':

                return strrev(trim($Value)); ;
        }
        return null;
    }
}
